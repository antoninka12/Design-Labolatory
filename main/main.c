...
{   //musi być w pętli głównej:
    ...
    ESP_ERROR_CHECK(oled_init(I2C_PORT)); //checking for errors
    ...
    //wyswietlanie różnych tekstów, tutaj juz mozesz sobie wybbierac które bedziesz potrzebowac, ESP_LOGI to logi z tagiem main, zeby wiedziec co sie dzieje, jak nie chcesz nie musisz z nich korzystac to wtedy 
    //bez linijki ESP_LOGI(TAG, "drawing CZEKAJ");, i wgl jesli bez sprawdzania bledow to bez ESP_ERROR_CHECK, tylko oled_draw_bitmap(I2C_PORT, czekaj_bitmap, czekaj_bitmap_len), ale jak cos sie zepsuje to latwiej wiedziec z logów
    //gdzie konkretnie jest błąd
    ESP_LOGI(TAG, "drawing CZEKAJ");
    ESP_ERROR_CHECK(oled_draw_bitmap(I2C_PORT, czekaj_bitmap, czekaj_bitmap_len));
    ...
    // leaving main task in infinite loop
    while (1) {
        vTaskDelay(pdMS_TO_TICKS(1000));
    }
}//opisy masz w plikach nagłówkowych funkcji 
